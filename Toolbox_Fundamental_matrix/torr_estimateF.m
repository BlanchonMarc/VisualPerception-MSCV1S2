...
case {'mlesac',0}
    ...
    [f,f_sq_errors, n_inliers,inlier_index] = torr_mlesac_F(x1,y1,x2,y2, no_matches, m3, no_samp, T);
...
case {'lin+non_lin',6}
    set_rank2 = 1;
    f = torr_estimateF(matches, no_matches, m3, 'linear',set_rank2);
    f = torr_estimateF(matches, no_matches, m3,  'non_linear',set_rank2,f);
   
case {'fns',7}
    f = ant_fnsf(x1,y1,x2,y2,m3);
    
case {'cfns',8}
    f = ant_cfnsf(x1,y1,x2,y2,m3);
...
switch lower(method)
case {'bookstein',3,'linear',2,'bookstein',3,'b+sampson','boosam',4,'non_linear',5,'lin+non_lin',6,'fns',7,'cfns',8}
