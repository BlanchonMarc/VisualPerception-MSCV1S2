%funmatLMedSls    Matriu Fonemantal amb el metode Least Median of Squares amb mínims quadrats
%    [F] = funmatLMedSls(M,b,P,ep) 
%
...
%
function [F,w]=funmatLMedSls(M,b,P,ep)
...
if (size(M,1)~=4) | (size(M,2)<8),
   ...
   for i=1:size(M,2),
      if w(i)~=0,
      	Up=[M(1,i)*M(3,i) M(1,i)*M(4,i) M(1,i) M(2,i)*M(3,i) M(2,i)*M(4,i) M(2,i) M(3,i) M(4,i)];      
         UU=[UU ; Up];         
      end      
   end
   fp=-(UU'*UU)^(-1)*UU'*ones(1,size(UU,1))';
